...
export const leaveApplicationSchema = z
  .object({
    ...
    startDate: z
      .string()
      .regex(/^\d{4}-\d{2}-\d{2}$/, 'Start date must be in mm/dd/yyyy format'),
    endDate: z
      .string()
      .regex(/^\d{4}-\d{2}-\d{2}$/, 'End date must be in mm/dd/yyyy format'),
    durations: z.coerce.number().min(1, 'Duration must be at least 1'),
    resumptionDate: z
      .string()
      .regex(
        /^\d{4}-\d{2}-\d{2}$/,
        'Resumption date must be in mm/dd/yyyy format',
      ),
